...
- `MainView`: primary single-window interface; switches between Podcasts and Playlists, and shows device status and the last completed sync
...
- `SettingsView`: larger settings sheet with General, Downloads, Device, and Advanced sidebar pages; one `SettingsDraft` retains unsaved preferences across navigation, and a shared Save/Cancel footer applies or discards the draft. Downloads groups MP3 metadata; Advanced contains FFmpeg, HTTP download permissions, and app data backup and restore; Device groups both device folders while keeping folder creation and migration confirmations intact. Pages use a bounded content width and consistently spaced fields; folder selectors stay compact. Save is enabled only when normalized draft values differ from the initial settings, device folders, or available update preference. Device context appears below the page heading; compact paths provide full-value tooltips and copying. Backup and restore actions are visually separated. Existing setting labels and explanatory wording are preserved except for the consolidated disconnected-device guidance.
...
- `SyncDialogView`: review the plan in one shared scrolling area. Planned actions start expanded; cleanup and playlist-protected episodes start collapsed, with counts in each disclosure heading. Expansion survives selection-driven replanning. Cleanup rows share the Podcast panel’s playlist menu; successful membership edits persist immediately and clear deletion selection. `SyncCleanupReview` keeps newly protected rows in their original position for the current review, while later reviews use the normal protected section. Failed playlist saves leave deletion selection unchanged. The summary, deletion notice, and trailing Close/Sync actions stay outside the scrolling content; selection and execution semantics are unchanged.
...
- `SyncCompletionNotifier`: request native notification permission when a reviewed sync starts and post a “Sync complete” notification with the standard macOS notification sound only when execution succeeds while the app is inactive. Failed attempts never announce success; notification denial or delivery errors never fail a sync. Unbundled development runs skip UserNotifications; banner verification requires the isolated packaged development app.
  At completion, the body summarizes nonzero copied, deleted, and verified already-on-device episode counts plus updated and deleted playlist counts, with each nonzero category on its own • bulleted line and “episode”/“episodes” included for copies and deletions. It adds a final bulleted line reading “Device ejected.” only after successful eject. Notification permission is checked again without prompting before comparing the frontmost process with this app's PID, falling back to AppKit's activation flag only when macOS cannot identify a frontmost app. Scoped system logs record permission, focus, and submission outcomes for delivery diagnosis.
...
- `SyncPlanner`: calculate copy, skip, selected per-podcast retention cleanup, manual delete, playlist write/delete, and eject actions; verify the complete plan fits; and order selected deletions before copies
...
7. `SyncPlanViewModel` builds the full-device plan:
   ...
   - build a `SyncPlan`
   - build UTF-8 M3U files from the projected post-sync device contents, using portable relative backslash paths
...
The episode grid combines the latest feed entries with locally prepared downloads for the selected podcast. It deduplicates by episode ID within that subscription, prefers current RSS metadata, and uses newest-first ordering. Downloaded episodes absent from known feed data receive the neutral “No longer in current feed” label and retain the usual local-download and device-removal controls. Loading or failed feed refreshes do not establish that an episode was removed. Download history without a local file adds no rows; after removing a retained local download, any device-only copy remains accessible in “Older episodes on MP3 player.” This merged list is for display and device matching; RSS activity and automatic-download decisions continue to use feed entries only.

The episode list supplies the complete merged catalogue to SwiftUI’s native `List` using stable episode identifiers. SwiftUI loads rows as needed while scrolling; there is no application-managed pagination, page-size preference, or load-more trigger.

Podcast activity is separate from automatic downloads. Existing podcasts establish a no-badge baseline, and only episodes ahead of a previously observed episode are normally considered new. Podcast selection does not change activity state. A successful manual or automatic download acknowledges that episode, and a fully successful sync acknowledges prepared episodes copied or already present. New-episode badges are withheld while a refresh and its automatic downloads are active, then the sidebar shows blue new-episode text only for episodes that remain unacknowledged; an orange inactive label is shown when applicable, and refresh errors take precedence. RSS feed URL changes establish a fresh baseline. Activity state is included in app-data backups.

The main view constructs its refresh coordinator from the currently retained SwiftUI state models when needed. It must not capture temporary model instances in the view initializer: SwiftUI can recreate the view while retaining its original state, leaving refresh results disconnected from the visible episode list.
...
Every successful sync finishes by removing verified AppleDouble sidecars matching new and retained SPM-managed episode files, before automatic eject when enabled. `SyncPlan` carries the existing ownership inventory so this also repairs devices when no downloads need copying; copy destinations are included and episodes selected for removal are excluded. Playlist writes and owned playlist removals also remove their exact matching sidecars. The executor validates exact paths, rejects symlinks and non-regular files, and verifies the AppleDouble version and entry bounds before deleting a matching `._` file. It does not scan or clean unrelated files, orphan episode sidecars, directories, or other device folders. A cleanup failure preserves completed transfers, reports incomplete sync, and prevents automatic eject. The copy implementation, embedded MP3 metadata, and separate manual-eject action are unchanged. This addresses reproduced sidecar-related playlist failures without imposing an unsupported filename-length limit. See [the AppleDouble sidecar investigation and design decision](docs/APPLEDOUBLE_SIDECAR_ISSUE.md) for the observations, limits of the diagnosis, and regression requirements.
...
- playlist definitions, ordered explicit episode snapshots, automatic settings and exclusions, accumulated recent-download snapshots, and per-device playlist ownership and tombstones
...
New app data backups are format-version-2 JSON directories. They include configuration, prepared/download/removal records, automatic-download state, podcast activity, and playlist definitions. Restore still accepts format-version-1 backups, treating the absent playlist file as an empty library, validates the complete backup before changing live data, and writes all episode state in one database transaction.

## Playlists

Playlists are available to all users through library navigation, episode actions, menus, and shortcuts. Playlist sync and cleanup protection always participate in planning. The retired `showsPlaylistsBeta` settings key is ignored when loading older configurations and omitted on save; playlist definitions and per-device ownership records remain unchanged. Playlists use one flexible model rather than separate manual and smart types. Every playlist can contain explicitly added episodes in a user-controlled order and can optionally add matching episodes automatically. Each definition also stores a user-selected icon from a small curated set; playlists saved before icon selection was introduced use the music-note default. The new-playlist and edit-playlist sheets can choose that icon and enable or disable automatic additions without creating a separate playlist type or affecting explicit entries.
...
The playlist editor exposes one automatic rule: choose one or more Podcasts whose downloaded episodes should be included, with an optional latest-episode count. The app display resolves persisted download snapshots even when no device is connected, preferring current feed metadata for episodes that are downloaded locally or matched on the selected device and adding read-only snapshots of older app-managed device files outside the current RSS window. Offline snapshot rows remain visible but show as unavailable when neither a local nor mounted-device copy can be played. Those entries appear after explicit entries, newest first across the combined Podcast selection, and remain subject to retention cleanup. A limit restricts how many automatic entries participate. Removing an automatic entry records a stable Podcast-and-file-stem exclusion so it does not immediately return, while explicitly adding it again clears that exclusion. Deleting a Podcast removes its entries, snapshots, exclusions, and automatic selection from every playlist; when no selected Podcasts remain, automatic additions are disabled and the playlist becomes manual-only.
...
Each nonempty playlist is encoded as UTF-8 extended M3U at `[configured playlist directory]/<playlist-name>.m3u`. The configured playlist directory defaults to the Podcast directory for compatibility with existing devices and can be changed per device in Settings beside the Podcast folder, including to a root-level folder such as `playlist_data`. Entries use backslash-separated paths calculated relative to the playlist file's directory, so the same encoding works when playlists and podcast media live in different folders. When none of a playlist's episodes are present or planned for copy, sync removes its SPM-owned M3U from the device but retains the playlist definition in the app; a later sync recreates the file after an episode becomes available. Before scheduling a write, the planner compares the encoded bytes with the validated, already-owned file in the current playlist directory. An exact match is retained without a write or an update count; missing, changed, or unreadable files still receive a planned write, and unowned collisions still stop planning. SPM records playlist filenames and the directory written or verified unchanged by a successful sync, preserving ownership for future updates and deletions. Renames, deletions, and emptying leave durable ownership state until a successful sync removes the old file. Ownership never transfers merely because the configured folder changes, so an unrelated same-named playlist in a new folder is not overwritten or removed. Playlist writes and removals delete only verified AppleDouble sidecars with the exact matching `._<playlist-name>.m3u` name.
...
Deleting a Podcast removes its in-memory RSS episodes, summaries, and errors without fetching the remaining feeds. A completed device inventory for the same device and Podcast directory is reduced to the remaining subscriptions and reused for planning; missing, in-progress, or incompatible inventories are refreshed. Retained device files from deleted Podcasts become candidates for an explicit other-audio review, never automatic deletion. Existing other-audio review results and selections are invalidated when ownership changes.
...
- finalize every MP3 with a small, deterministic ID3v2.3 tag using the RSS episode and podcast titles, the configured genre when nonblank (defaulting to `Podcast`), plus prepared cover art when available
- Settings can prefix `TIT2` with the RSS publication date in fixed `MM.dd` format; this applies only to new downloads
- Settings can customize the `TCON` genre written to new downloads or leave it blank to omit the frame
