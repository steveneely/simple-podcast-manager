...
Playlists are temporarily exposed as an opt-in beta through **Settings → Beta Features → Enable Playlists**. The setting defaults off and gates playlist navigation, episode actions, menus, shortcuts, recent-download tracking, sync actions, and cleanup protection. Disabling the beta preserves playlist definitions and per-device ownership state, but sync treats the library as empty: it neither writes nor deletes playlist files, and existing playlist files on MP3 players remain unchanged. Essential episode and Podcast deletion cleanup continues so preserved definitions remain valid if the feature is re-enabled. Playlists use one flexible model rather than separate manual and smart types. Every playlist can contain explicitly added episodes in a user-controlled order and can optionally add matching episodes automatically. Each definition also stores a user-selected icon from a small curated set; playlists saved before icon selection was introduced use the music-note default. The new-playlist and edit-playlist sheets can choose that icon and enable or disable automatic additions without creating a separate playlist type or affecting explicit entries.
