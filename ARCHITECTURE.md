...
Refresh and download status have distinct presentation responsibilities. The lower-left podcast footer owns library-level refresh progress and the durable outcome: completed-versus-total podcast checks while feeds are loading, the count of episodes found during that refresh, the current still-new count, and an expandable final summary of downloads, still-new episodes, and actionable refresh or download failures. Successful manual downloads reconcile that durable outcome immediately by moving the matching episode from still-new to downloaded. Its semantic colors follow the rest of the app: blue for still-new state, green for successful downloads, orange for warnings, and neutral text for checked and discovered counts. The status beside the Device section owns only active media preparation and download work and disappears when that work finishes. Do not duplicate live download progress in the podcast footer or rely on transient per-podcast badge changes to explain a large-library refresh.
