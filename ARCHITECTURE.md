...
Automatic device refresh inventories only the immediate app-managed directory for each subscription. It does not recursively walk the configured podcast directory or collect unrelated audio. The resulting managed inventory is reused for episode status and sync-plan rebuilds until the device, configured podcast directory, or subscription set changes. External players can have slow storage, so inventory and planning filesystem reads run outside the main actor, obsolete work is cancelled, and closely spaced device-topology notifications are coalesced.

Other audio is reviewed only when the user chooses `Review Files in Podcast Folder`. That cancellable background operation recursively inventories the configured podcast directory, excludes files from the current managed inventory, and reports progress without blocking the UI. The Other Audio section appears only when the completed review finds files; an empty review leaves the section hidden. Large result sets render lazily. Disconnecting, changing the configured directory, or refreshing the device invalidates the review.
