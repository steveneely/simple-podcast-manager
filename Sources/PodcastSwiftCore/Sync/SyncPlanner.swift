...
public struct SyncPlanner: Sendable {
    ...
    private func managedDirectoryURL(for subscription: FeedSubscription, on device: DeviceInfo) -> URL {
        let folderName = (subscription.podcastFolderName?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
            ? subscription.podcastFolderName!
            : subscription.title
        return device.musicURL.appendingPathComponent(folderName, isDirectory: true)
    }
}
