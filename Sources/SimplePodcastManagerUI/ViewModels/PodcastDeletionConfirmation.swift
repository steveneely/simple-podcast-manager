...
struct PodcastDeletionConfirmation: Equatable {
    ...
    init(
        ...
        if subscriptions.count == 1, let subscription = subscriptions.first {
            ...
            message = "Are you sure you want to delete “\(subscription.title)”?\(localDownloadMessage)\(playlistMessage)\(deviceMessage)"
            deleteButtonTitle = "Delete Podcast"
        } else {
            title = "Delete Podcasts?"
            message = "Are you sure you want to delete these \(subscriptions.count) podcasts?\(localDownloadMessage)\(playlistMessage)\(deviceMessage)"
