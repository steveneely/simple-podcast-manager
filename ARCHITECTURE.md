...
- `SettingsView`: larger settings sheet with General, Downloads, Device, and Advanced sidebar pages; one `SettingsDraft` retains unsaved preferences across navigation, and a shared Save/Cancel footer applies or discards the draft. Downloads groups MP3 metadata; Advanced contains FFmpeg, HTTP download permissions, and app data backup and restore; Device groups both device folders while keeping folder creation and migration confirmations intact. Pages use a bounded content width and consistently spaced fields; folder selectors stay compact. Existing setting labels and explanatory wording are preserved.
